...
let consequences = [
    [
        ...
        [
            "O responsável não tomar a situação a sério, o que faz João se sentir desamparado. Isso pode até fazer com que o bullying se intensifique, já que o agressor sente que não há consequências reais para seu comportamento. Ítalo fica ainda mais frustrado e perde a confiança nas pessoas ao seu redor.",
            "O responsável escuta João e tenta ajudar, mas a solução é apenas parcial. As provocações diminuam um pouco, não há uma resolução completa. ",
...
function UpdateQuestion () {
    ...
    document.getElementById("questionNumberId").innerHTML =  "- ["+currentQuestion+"/10] -"
}
...
async function AnswerQuestion(buttonPressed) {
    if (!answered){
        ...
        else if (diceValue > 7) consequenceText.innerHTML = consequences[currentQuestion-1][buttonPressed][1];
